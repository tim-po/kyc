...
const StandardAppContainer = (props: { logoHref?: string, hideWalletConnector?: boolean, children: any, locales: string[], isDarkBG?: boolean, version: string, pages?: { title: string, url: string }[] }) => {
  ...
  const AccentedButtonVisualContext = styled.div<{accentedButton: number}>`
    ${(props)=> ((props.accentedButton && props.accentedButton !== -1) && css`
      .connection-button:nth-child(${props.accentedButton}) {
  ...
  useEffect(() => {
    ...
    return (<ConfigProvider getPopupContainer={trigger => trigger.parentElement}>
            <LocaleContext.Provider value={{ setLocale, locale }}>
              <AccentedButtonVisualContext accentedButton={accentedControlButton}>
                <WalletConnectorBubbleContext.Provider value={{setBubbleValue: setBubbleValue, bubbleValue: bubbleValue, setAccentedControlButton: changeAccentedButton}}>
